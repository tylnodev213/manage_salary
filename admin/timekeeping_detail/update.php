...
$nam = $_GET['nam'] ?? date("Y");
include '../conn.php';
$sql = "SELECT * FROM chamcong_chitiet WHERE id = $id";
$result = mysqli_query($connect,$sql);
include '../dis-conn.php';
foreach ($result as $each){
    echo '
    <form action="process_update.php?p='.$p.'&search='.$search.'&thang='.$thang.'&nam='.$nam.'" method="post">
    <input type="hidden" name="id" value="'. $each['id'].'">
    <label for="example">Ngày chấm công</label>
    <input placeholder="Select date" type="date" class="form-control" name="ngayChamCong" value="'. $each['ngayChamCong'].'">
