<h5 class="title">Danh sách chức vụ</h5>
<div class="header_table">
    <a class="btn btn-success" href="?action=create">Tạo chức vụ mới</a>
    ...
    <form action="" method="get" class="input-group">
        ...
        ?>
        <input type="search" id="form1" class="form-control" name="search" value="<?php echo $search;?>" placeholder="Tìm chức vụ">
...
<thead class="thead-dark">
    ...
    $sql_tong_ban_ghi = "SELECT COUNT(id)
            AS tong_so_ban_ghi
            FROM chucVu
            WHERE chucVu.chucVu LIKE '%$search%'";
    ...
    $sql= "SELECT * from chucVu WHERE chucVu  LIKE '%$search%'
    ...
    <tr>
        ...
        WHERE idChucVu = ".$each['id']."";
        $result = mysqli_query($connect, $sql);
        include 'dis-conn.php';
            if(mysqli_num_rows($result)==0){
                ?>
                <a class="btn btn-danger" href="position/delete.php?id=<?php echo $each['id']?>&search=<?php echo $search; ?>&p=<?php echo $p; ?>"><i class="fa-solid fa-trash-can"></i></a>
                <?php
            }
        ?>
        </td>
