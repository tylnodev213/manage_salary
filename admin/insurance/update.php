...
foreach ($result as $each){
    echo '
    <form class="title" action="insurance/process_update.php?search='.$search.'&p='.$p.'" method="post">
    <input type="hidden" name="id" value="'. $each['id'].'">
    Loại bảo hiểm: <input class="form-control" type="text" name="name_insurance" value="'. $each['loaiBaohiem'].'"><br>
    Tiền bảo hiểm(%): <input class="form-control" type="number" step="0.01" name="money_insurance" value="'. $each['tienBaohiem'].'">
    <button class="btn btn-primary">Sửa</button>
    </form>';
}
?>
<span class="title" style="font-size:12px; color:red" id="error_length"></span>
