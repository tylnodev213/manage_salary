...
foreach ($result as $each){
    echo '
    <form class="title" action="process_update.php?search='.$search.'&p='.$p.'" method="post">
    <input type="hidden" name="id" value="'. $each['id'].'">
    Tên phòng: <input class="form-control" id="name" type="text" name="name_department" value="'. $each['tenPhong'].'"><br>
    <button class="btn btn-primary">Sửa</button>
    </form>';
}
?>
<span style="font: size 12px;color:red;" id="error_length"></span>
<span style="font: size 12px;color:red;" id="error_name"></span>
<?php
include '../jQuery.php';
?>
<script>
